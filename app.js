...
const BACKEND_DOMAIN = "restaurante-sistema-qfxa.onrender.com"; 
...
async function loadDashboard() {
    ...
    try {
        const res = await fetch(`${API_BASE}/dashboard/${RESTAURANT_ID}`);
        if(res.ok) {
...
async function loadShiftAdmin() {
    ...
    try {
        const res = await fetch(`${API_BASE}/shifts/${RESTAURANT_ID}/current`);
...
async function openShift() {
    ...
    try {
        const res = await fetch(`${API_BASE}/shifts/${RESTAURANT_ID}/open`, {
            method: 'POST',
            headers: {'Content-Type': 'application/json'},
...
async function closeShift() {
    ...
    try {
        const res = await fetch(`${API_BASE}/shifts/${RESTAURANT_ID}/close/${CURRENT_SHIFT_ID}`, {
            method: 'POST',
            headers: {'Content-Type': 'application/json'},
...
async function loadTablesGrid() {
    ...
    if(!container) return;
    try {
        let res = await fetch(`${API_BASE}/tables/${RESTAURANT_ID}/status`);
        let tables = [];
        
        // Fallback: Si el usuario aún no actualiza el backend en Render, buscar tablas normales
        if (!res.ok) {
            res = await fetch(`${API_BASE}/tables/${RESTAURANT_ID}`);
